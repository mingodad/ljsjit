...
// Arch-specific maps.
var map_archdef = { sp = "r1" }; // Ext. register name -> int. name.
...
map_op = {
  ...
  subi_3 =	op_alias("addi_3", function(p) { p[3] = "-("..p[3]..")"; }),
  subis_3 =	op_alias("addis_3", function(p) { p[3] = "-("..p[3]..")"; }),
  subic_3 =	op_alias("addic_3", function(p) { p[3] = "-("..p[3]..")"; }),
  ["subic._3"] = op_alias("addic._3", function(p) { p[3] = "-("..p[3]..")"; }),
...
// Add mnemonics for "." variants.
{
  ...
  for( k,v in pairs(map_op) ) {
    if( type(v) == "string" && sub(v, -1) == "." ) {
      var v2 = sub(v, 1, 7)..char(byte(v, 8)+1)..sub(v, 9, -2);
...
for( cond,c in pairs(map_cond) ) {
  var b1 = "b"..cond;
  var c1 = shl(band(c, 3), 16) + (c < 4 && 0x01000000 || 0);
  // bX[l]
...
var function parse_vr(expr) {
  var r = match(expr, "^v([1-3]?[0-9])$");
  if( r ) {
    r = tonumber(r);
    if( r <= 31 ) { return r; }
  }
...
var function parse_vs(expr) {
  var r = match(expr, "^vs([1-6]?[0-9])$");
  if( r ) {
    r = tonumber(r);
    if( r <= 63 ) { return r; }
  }
...
var function parse_cr(expr) {
  var r = match(expr, "^cr([0-7])$");
  if( r ) { return tonumber(r); }
  werror("bad condition register name `"..expr.."'");
}

var function parse_cond(expr) {
  var r, cond = match(expr, "^4%*cr([0-7])%+(%w%w)$");
  if( r ) {
    r = tonumber(r);
    var c = map_cond[cond];
    if( c && c < 4 ) { return r*4+c; }
  }
...
var function parse_imm(imm, bits, shift, scale, signed) {
  ...
  } else if( match(imm, "^[rfv]([1-3]?[0-9])$") ||
	 match(imm, "^vs([1-6]?[0-9])$") ||
	 match(imm, "^([%w_]+):(r[1-3]?[0-9])$") ) {
...
var function parse_shiftmask(imm, isshift) {
  var n = parse_number(imm);
  if( n ) {
    if( shr(n, 6) == 0 ) {
      var lsb = band(n, 31);
      var msb = n - lsb;
      return isshift && (shl(lsb, 11)+shr(msb, 4)) || (shl(lsb, 6)+msb);
    }
    werror("out of range immediate `"..imm.."'");
  } else if( match(imm, "^r([1-3]?[0-9])$") ||
	 match(imm, "^([%w_]+):(r[1-3]?[0-9])$") ) {
    werror("expected immediate operand, got register");
  } else {
    waction("IMMSH", isshift && 1 || 0, imm);
    return 0;
  }
}
...
var function parse_disp(disp) {
  ...
  if( imm ) {
    var r = parse_gpr(reg);
    if( r == 0 ) { werror("cannot use r0 in displacement"); }
...
var function parse_u5disp(disp, scale) {
  var imm, reg = match(disp, "^(.*)%(([%w_:]+)%)$");
  if( imm ) {
    var r = parse_gpr(reg);
    if( r == 0 ) { werror("cannot use r0 in displacement"); }
...
op_template = function(params, template, nparams) {
  ...
  for( p in gmatch(sub(template, 9), ".") ) {
    if( p == "R" ) {
      rs -=   5; op +=   shl(parse_gpr(params[n]), rs); n +=   1;
    } else if( p == "F" ) {
      rs -=   5; op +=   shl(parse_fpr(params[n]), rs); n +=   1;
    } else if( p == "V" ) {
      rs -=   5; op +=   shl(parse_vr(params[n]), rs); n +=   1;
    } else if( p == "Q" ) {
      var vs = parse_vs(params[n]); n +=   1; rs -=   5;
      var sh = rs == 6 && 2 || 3 + band(shr(rs, 1), 3);
      op +=   shl(band(vs, 31), rs) + shr(band(vs, 32), sh);
    } else if( p == "q" ) {
      var vs = parse_vs(params[n]); n +=   1;
      op +=   shl(band(vs, 31), 21) + shr(band(vs, 32), 5);
    } else if( p == "A" ) {
      rs -=   5; op +=   parse_imm(params[n], 5, rs, 0, false); n +=   1;
    } else if( p == "S" ) {
      rs -=   5; op +=   parse_imm(params[n], 5, rs, 0, true); n +=   1;
    } else if( p == "I" ) {
      ...
      op +=   parse_u5disp(params[n], 1); n +=   1;
    } else if( p == "4" ) {
      op +=   parse_u5disp(params[n], 2); n +=   1;
    } else if( p == "8" ) {
      op +=   parse_u5disp(params[n], 3); n +=   1;
    } else if( p == "C" ) {
      rs -=   5; op +=   shl(parse_cond(params[n]), rs); n +=   1;
    } else if( p == "X" ) {
      rs -=   5; op +=   shl(parse_cr(params[n]), rs+2); n +=   1;
    } else if( p == "1" ) {
      rs -=   5; op +=   parse_imm(params[n], 1, rs, 0, false); n +=   1;
    } else if( p == "g" ) {
      rs -=   5; op +=   parse_imm(params[n], 2, rs, 0, false); n +=   1;
    } else if( p == "3" ) {
      rs -=   5; op +=   parse_imm(params[n], 3, rs, 0, false); n +=   1;
    } else if( p == "P" ) {
      rs -=   5; op +=   parse_imm(params[n], 4, rs, 0, false); n +=   1;
    } else if( p == "p" ) {
      op +=   parse_imm(params[n], 4, rs, 0, false); n +=   1;
    } else if( p == "6" ) {
      rs -=   6; op +=   parse_imm(params[n], 6, rs, 0, false); n +=   1;
    } else if( p == "Y" ) {
      rs -=   5; op +=   parse_imm(params[n], 1, rs+4, 0, false); n +=   1;
    } else if( p == "y" ) {
      rs -=   5; op +=   parse_imm(params[n], 1, rs+3, 0, false); n +=   1;
    } else if( p == "Z" ) {
      rs -=   5; op +=   parse_imm(params[n], 2, rs+3, 0, false); n +=   1;
    } else if( p == "z" ) {
      rs -=   5; op +=   parse_imm(params[n], 2, rs+2, 0, false); n +=   1;
    } else if( p == "W" ) {
      op +=   parse_cr(params[n]); n +=   1;
    } else if( p == "G" ) {
      op +=   parse_imm(params[n], 8, 12, 0, false); n +=   1;
    } else if( p == "H" ) {
      op +=   parse_shiftmask(params[n], true); n +=   1;
    } else if( p == "M" ) {
      op +=   parse_shiftmask(params[n], false); n +=   1;
    } else if( p == "J" || p == "K" ) {
      ...
      if( band(shr(op, rs), 31) == 0 ) { werror("cannot use r0"); }
    } else if( p == "=" || p == "%" ) {
      var t = band(shr(op, p == "%" && rs+5 || rs), 31);
      rs -=   5;
      op +=   shl(t, rs);
    } else if( p == "~" ) {
      var mm = shl(31, rs);
      var lo = band(op, mm);
      var hi = band(op, shl(mm, 5));
      op -=   lo - hi + shl(lo, 5) + shr(hi, 5);
    } else if( p == ":" ) {
      if( band(shr(op, rs), 1) != 0 ) { werror("register pair expected"); }
    } else if( p == "-" ) {
      rs -=   5;
    } else if( p == "." ) {
      // Ignored.
