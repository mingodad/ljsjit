...
/* Lua lexer tokens. */
#define TKDEF(_, __) \
  _(auto) _(break) _(continue) _(do) _(else) _(false) \
   __(file_line, __LINE__) __(file_name, __FILE__) \
  _(for) _(function) _(goto) _(if) _(in) _(let) _(local) _(null) _(not) \
  _(repeat) _(return) _(true) _(var) _(while) \
  __(and, &&) __(or, ||) \
  __(concat, ..) __(dots, ...) __(eq, ==) __(ge, >=) __(le, <=) __(ne, !=) \
  __(cadd, +=) __(csub, -=) __(cmul, *=) __(cdiv, /=) __(cmod, %=) \
  __(plusplus, ++) __(minusminus, --) \
  __(cconcat, ..=) __(pow, **)  __(arrow, ->)\
  __(dbcolon, ::) __(number, <number>) __(name, <name>) __(string, <string>) \
  __(eof, <eof>)
