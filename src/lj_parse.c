...
static void gola_fixup(LexState *ls, FuncScope *bl)
{
  ...
  for (; v < ve; v++) {
	...
	if (bl->prev) {  /* Propagate goto, break, continue to outer scope. */
...
static void expr_table_array(LexState *ls, ExpDesc *e, LexToken openTk, LexToken closeTk)
{
  FuncState *fs = ls->fs;
  ...
  freg++;
  lex_check(ls, openTk);
  while (ls->tok != closeTk) {
    ExpDesc key, val;
    vcall = 0;
    if(openTk == '[') goto array_decl;
    if (ls->tok == '[') {
  ...
  lex_match(ls, closeTk, openTk, line);
  if (vcall) {
...
{
  expr_table_array(ls, e, '{', '}');
}

/* Parse array constructor expression. */
static void expr_array (LexState *ls, ExpDesc *e) {
  /* array -> '[' [ item { ',' item } [ ',' ] ] ']' */
  expr_table_array(ls, e, '[', ']');
}

/* Parse function parameters. */
static BCReg parse_params(LexState *ls, int needthis)
{
  FuncState *fs = ls->fs;
  BCReg nparams = 0;
  lex_check(ls, '(');
  if (needthis)
    var_new_lit(ls, nparams++, "this");
  if (ls->tok != ')') {
    do {
      if (ls->tok == TK_name || (!LJ_52 && ls->tok == TK_goto)) {
      ...
      if(lex_opt(ls, ':')) { /*optional parameter type*/
        lex_check(ls, TK_name); /*for now do nothing, discard*/
      }
    } while (lex_opt(ls, ','));
  }
  var_add(ls, nparams);
...
static void parse_body(LexState *ls, ExpDesc *e, int needthis, BCLine line)
{
  ...
  fs.linedefined = line;
  fs.numparams = (uint8_t)parse_params(ls, needthis);
  if(lex_opt(ls, ':')) { /*optional function return type*/
    lex_check(ls, TK_name); /*for now do nothing, discard*/
  }
  lex_check(ls, '{');
  ...
  parse_chunk(ls);
  if (ls->tok != '}') lex_match(ls, '}', TK_function, line);
...
static void expr_simple(LexState *ls, ExpDesc *v)
{
  ...
  case TK_file_line:
    expr_init(v, VKNUM, 0);
    setintV(&v->u.nval, ls->linenumber);
    break;
  case TK_file_name:
    expr_init(v, VKSTR, 0);
    v->u.sval = ls->chunkname;
    break;
  default:
    expr_primary(ls, v);
    return;
...
static BCPos expr_nocond (LexState *ls) {
  /* cond -> exp */
  ExpDesc v;
  expr(ls, &v);  /* read condition */
  if (v.k == VKNIL) v.k = VKFALSE;  /* 'falses' are all equal here */
  bcemit_branch_f(ls->fs, &v);
  return v.t;
}
...
static void parse_local(LexState *ls)
{
  if (lex_opt(ls, TK_function)) {  /* Local function declaration. */
    ...
    do {  /* Collect LHS. */
      ...
      if(lex_opt(ls, ':')) { /*optional var type*/
        lex_check(ls, TK_name); /*for now do nothing, discard*/
      }
    } while (lex_opt(ls, ','));
...
static void parse_func(LexState *ls, BCLine line)
{
  ...
  while (ls->tok == '.')  /* Multiple dot-separated fields. */
    expr_field(ls, &v);
  if (ls->tok == TK_dbcolon) {  /* Optional -> to signify method call. */
    needthis = 1;
    expr_field(ls, &v);
  }
  parse_body(ls, &b, needthis, line);
  fs = ls->fs;
  bcemit_store(fs, &v, &b);
...
static void parse_label(LexState *ls)
{
  ...
  //What this mean now with do_while ???
  //if (parse_isend(ls->tok) && ls->tok != TK_while)
...
static void parse_do_while(LexState *ls, BCLine line)
{
  FuncState *fs = ls->fs;
