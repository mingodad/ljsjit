  ...
  for (;;) {
    ...
    case '/':
        ...
        if (ls->c == '/') {
          ...
          if (ls->c != '=') return '/';
          else { lex_next(ls); return TK_cdiv; }
        }
      end_long_comment:
      break;
    case '[': {
    ...
    case '-':
      ...
      if (ls->c == '-')  { lex_next(ls); return TK_minusminus; }
      return '-';
    case '*':
      lex_next(ls);
      if (ls->c == '=') { lex_next(ls); return TK_cmul; }
      if (ls->c != '*') return '*'; else { lex_next(ls); return TK_pow; }
    case '%':
      lex_next(ls);
      if (ls->c != '=') return '%'; else { lex_next(ls); return TK_cmod; }
    case '&':
      lex_next(ls);
    ...
    case ':':
      lex_next(ls);
      if (ls->c != ':') return ':'; else { lex_next(ls); return TK_dbcolon; }
    case '"':
    case '\'':
      lex_string(ls, tv);
      return TK_string;
